...
window['app'] = new Vue({
    ...
    mounted() {
        ...
        if (window.drop.send_at) {
            this.dropForm.send_at_date = moment(window.drop.send_at, 'YYYY-MM-DD HH:mm:ss').toDate();
            this.dropForm.send_at_time = moment(window.drop.send_at, 'YYYY-MM-DD HH:mm:ss').toDate();
        }
    },
    methods: {
